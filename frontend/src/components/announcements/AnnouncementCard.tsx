  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all duration-200 hover:shadow-md ${
        isUnread ? 'border-primary-300 bg-primary-50/50' : 'border-neutral-200'
      } ${announcement.is_pinned ? 'ring-2 ring-warning-400' : ''}`}
    >
      <div className="p-4 md:p-6">
        {/* Header */}
        <div className="flex items-start gap-4 mb-3">
          {/* Type Icon */}
          <div className={`p-2 rounded-xl ${PRIORITY_COLORS[announcement.priority]} flex-shrink-0`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Link
                ...
                className={`text-lg font-semibold hover:text-primary-600 transition-all duration-200 truncate ${
                  isUnread ? 'text-primary-700' : 'text-neutral-900'
                }`}
              >
                {announcement.title}
              </Link>
        ...
        {/* Content Preview */}
        {!compact && (
          <p className="text-neutral-600 text-sm mb-4 line-clamp-2">{announcement.content}</p>
        )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4 text-sm text-neutral-500">
            ...
            {announcement.expires_at && (
              <span className={announcement.is_expired ? 'text-error-500' : ''}>
          ...
          <div className="flex items-center gap-2">
            {isUnread && onMarkAsRead && (
              <button
                ...
                className="text-sm text-primary-600 hover:text-primary-700 transition-all duration-200"
              >
                تحديد كمقروء
              </button>
            )}
            ...
            {isTeacher && (
              <>
                ...
                <Link
                  ...
                  className="text-sm text-neutral-600 hover:text-neutral-700 transition-all duration-200"
                >
                  تعديل
                </Link>
                {!announcement.is_published && onPublish && (
                  <button
                    ...
                    className="text-sm text-success-600 hover:text-success-700 transition-all duration-200"
                  >
                    نشر
                  </button>
                )}
                {announcement.is_published && onUnpublish && (
                  <button
                    ...
                    className="text-sm text-accent-600 hover:text-accent-700 transition-all duration-200"
                  >
                    إلغاء النشر
                  </button>
                )}
                {!announcement.is_pinned && onPin && (
                  <button
                    ...
                    className="text-sm text-warning-600 hover:text-warning-700 transition-all duration-200"
                  >
                    تثبيت
                  </button>
                )}
                ...
                {onDelete && (
                  <button
                    ...
                    className="text-sm text-error-600 hover:text-error-700 transition-all duration-200"
                  >
                    حذف
                  </button>
                )}
