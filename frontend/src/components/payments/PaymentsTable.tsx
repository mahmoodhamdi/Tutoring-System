'use client';

import Link from 'next/link';
import { Payment } from '@/types/payment';
import { format } from 'date-fns';
import { arSA } from 'date-fns/locale';
import { TrashIcon, EyeIcon, CurrencyDollarIcon } from '@heroicons/react/24/outline';

interface PaymentsTableProps {
  payments: Payment[];
  onDelete?: (payment: Payment) => void;
  isDeleting?: boolean;
}

export function PaymentsTable({ payments, onDelete, isDeleting }: PaymentsTableProps) {
  const statusColors: Record<string, string> = {
    paid: 'bg-success-100 text-success-700',
    pending: 'bg-accent-100 text-accent-700',
    partial: 'bg-primary-100 text-primary-700',
    refunded: 'bg-neutral-100 text-neutral-600',
  };

  if (payments.length === 0) {
    return (
      <div className="text-center py-14">
        <div className="h-16 w-16 bg-neutral-100 rounded-full flex items-center justify-center mx-auto">
          <CurrencyDollarIcon className="h-8 w-8 text-neutral-400" />
        </div>
        <h3 className="mt-3 text-sm font-semibold text-neutral-900">لا توجد مدفوعات</h3>
        <p className="mt-1 text-sm text-neutral-500">ابدأ بتسجيل دفعة جديدة</p>
        <div className="mt-6">
          <Link
            href="/dashboard/payments/new"
            className="inline-flex items-center rounded-xl bg-gradient-to-l from-primary-600 to-primary-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:from-primary-700 hover:to-primary-600 transition-all duration-200"
          >
            تسجيل دفعة
          </Link>
        </div>
      </div>
    );
  }

  return (
    <div className="overflow-hidden rounded-xl border border-neutral-100 shadow-sm">
      <table className="min-w-full divide-y divide-neutral-100">
        <thead className="bg-neutral-50">
          <tr>
            <th className="py-3.5 pr-4 pl-3 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">الطالب</th>
            <th className="px-3 py-3.5 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">المبلغ</th>
            <th className="px-3 py-3.5 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">الفترة</th>
            <th className="px-3 py-3.5 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">تاريخ الدفع</th>
            <th className="px-3 py-3.5 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">الطريقة</th>
            <th className="px-3 py-3.5 text-right text-xs font-semibold text-neutral-500 uppercase tracking-wider">الحالة</th>
            <th className="relative py-3.5 pr-3 pl-4">
              <span className="sr-only">إجراءات</span>
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-neutral-100 bg-white">
          {payments.map((payment, idx) => (
            <tr
              key={payment.id}
              className={`hover:bg-neutral-50 transition-colors ${idx % 2 === 1 ? 'bg-neutral-50/50' : ''}`}
            >
              <td className="whitespace-nowrap py-4 pr-4 pl-3">
                <div className="font-semibold text-neutral-900">{payment.student?.name}</div>
                <div className="text-sm text-neutral-500">{payment.student?.phone}</div>
              </td>
              <td className="whitespace-nowrap px-3 py-4 text-sm font-bold text-neutral-900">
                {payment.amount.toLocaleString('ar-EG')} ج.م
              </td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-neutral-600">
                {payment.period}
              </td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-neutral-600">
                {payment.payment_date
                  ? format(new Date(payment.payment_date + 'T00:00:00'), 'd MMM yyyy', { locale: arSA })
                  : '-'}
              </td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-neutral-600">
                {payment.payment_method_label}
              </td>
              <td className="whitespace-nowrap px-3 py-4 text-sm">
                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusColors[payment.status]}`}>
                  {payment.status_label}
                </span>
              </td>
              <td className="relative whitespace-nowrap py-4 pr-3 pl-4 text-left text-sm font-medium">
                <div className="flex items-center gap-2">
                  <Link
                    href={`/dashboard/payments/${payment.id}`}
                    className="p-1.5 text-neutral-500 hover:text-neutral-800 hover:bg-neutral-100 rounded-lg transition-all duration-200"
                    title="عرض"
                  >
                    <EyeIcon className="h-4 w-4" />
                  </Link>
                  {onDelete && (
                    <button
                      onClick={() => onDelete(payment)}
                      disabled={isDeleting}
                      className="p-1.5 text-error-500 hover:text-error-700 hover:bg-error-50 rounded-lg transition-all duration-200 disabled:opacity-50"
                      title="حذف"
                    >
                      <TrashIcon className="h-4 w-4" />
                    </button>
                  )}
                </div>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
}

export default PaymentsTable;
